...
#### Load the Model
sc      = joblib.load("Standard_knnreg.pickle")
knnreg  = joblib.load("KNearest_Regressor.pickle")


#### Write the title of the Streamlit Application
st.title("K Nearest Regressor")

#### Write the contents in the Streamlit Application
st.write("Predict the House Price based on the Housing features using K Nearest Regressor")
...
if(st.button("Predict Housing Price")):
    ...
    predictions = knnreg.predict(scaled_input)
