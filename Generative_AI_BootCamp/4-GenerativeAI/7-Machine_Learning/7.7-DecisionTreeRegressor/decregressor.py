...
#### Load the Model
sc = joblib.load("Standard_decreg.pickle")
decreg = joblib.load("DecisionTreeRegressor.pickle")
...
st.write("Predict the House Price based on the Housing features using Decision Tree Regressor")
...
if(st.button("Predict HousePrice")):
    #### Convert all the input into 2D array
    inputdata = np.array([[crim, zn, indus, chas, nox, rm, age, dis, rad, tax, ptratio, b, lstat]])

    #### Scale the inputs
    scaled_input_data = sc.transform(inputdata)

    #### using the scaled data , predict the house price
    predictions = decreg.predict(scaled_input_data)
